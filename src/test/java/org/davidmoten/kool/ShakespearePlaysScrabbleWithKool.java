...
public class ShakespearePlaysScrabbleWithKool extends ShakespearePlaysScrabble {
    ...
    public List<Entry<Integer, List<String>>> measureThroughput() throws InterruptedException {
        ...
        Function<String, Stream<Integer>> toIntegerObservable = string -> Stream
                .from(IterableSpliterator.of(string.chars().boxed().spliterator()));

        // Histogram of the letters in a given word
        Function<String, Stream<HashMap<Integer, LongWrapper>>> histoOfLetters = word -> Stream.of(toIntegerObservable
                .apply(word).collect(() -> new HashMap<>(), (HashMap<Integer, LongWrapper> map, Integer value) -> {
        ...
        Function<Entry<Integer, LongWrapper>, Stream<Long>> blank = entry -> Stream
        ...
        Function<String, Stream<Long>> nBlanks = word -> histoOfLetters.apply(word)
                .flatMap(map -> Stream.from(map.entrySet())).flatMap(blank).reduce(Long::sum).toStream();
        ...
        Function<String, Stream<Integer>> score2 = word -> histoOfLetters.apply(word)
                .flatMap(map -> Stream.from(map.entrySet())).flatMap(letterScore).reduce(Integer::sum).toStream();
        ...
        Function<String, Stream<Integer>> score3 = word -> Stream
                ...
                .flatMap(observable -> observable).reduce(Integer::sum).toStream();

        Function<Function<String, Stream<Integer>>, Stream<TreeMap<Integer, List<String>>>> buildHistoOnScore = score -> Stream
                .of(Stream.from(shakespeareWords).filter(scrabbleWords::contains)
        ...
        List<Entry<Integer, List<String>>> finalList2 = buildHistoOnScore.apply(score3)
                ...
                .collect(() -> new ArrayList<Entry<Integer, List<String>>>(), (list, entry) -> {
                    list.add(entry);
                });
